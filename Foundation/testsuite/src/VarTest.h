...
private:
	...
	void testLimitsSigned()
	{
		...
		try { TS POCO_UNUSED i; i = da.convert<TS>(); failmsg("must fail"); }
	...
	void testLimitsSignedUnsigned()
	{
		...
		try { TU POCO_UNUSED i; i = dMin.convert<TU>(); failmsg("must fail"); }
		catch (Poco::RangeException&) {}
		...
		if(sizeof(TS) == sizeof(TU))
		{
			...
			try { TS POCO_UNUSED i; i = dMax.convert<TS>(); failmsg("must fail"); }
			catch (Poco::RangeException&) {}
		}
	}
