...
void LoggingConfiguratorTest::testConfigurator()
{
	...
	assertFalse ( pFC == nullptr );
#if defined(_WIN32) && !defined(_WIN32_WCE)
	assertFalse ( dynamic_cast< Poco::WindowsConsoleChannel * >( pFC->getChannel() ) == nullptr );
#else
	assertFalse ( dynamic_cast< ConsoleChannel * >( pFC->getChannel() ) == nullptr );
#endif
	assertFalse ( dynamic_cast< PatternFormatter * >( pFC->getFormatter() ) == nullptr );
	...
	assertFalse ( pFC == nullptr );
	assertFalse ( dynamic_cast< FileChannel * >( pFC->getChannel() ) == nullptr );
	assertFalse ( dynamic_cast< PatternFormatter * >( pFC->getFormatter() ) == nullptr );
	assert (static_cast<PatternFormatter*>(pFC->getFormatter())->getProperty("pattern") == "%s: [%p] %t");

	Logger& logger2 = Logger::get("logger2");
	assert (logger2.getLevel() == Message::PRIO_DEBUG);
	assertFalse ( dynamic_cast< SplitterChannel * >( logger2.getChannel() ) == nullptr );
}
